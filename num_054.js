...
function checkSeqNum(array){
    //오름차순 정렬
    const sortedArray = array.sort((prev, next) => prev - next);

    // console.log(sortedArray);

    //연속된 숫자의 첫번째 sequence number
    let seqNum = sortedArray[0];
    for(let i = 0; i < sortedArray.length; i++){
        //연속된 번호가 X -> false
        if(seqNum !== sortedArray[i]){
            return false;
        }
        //seqNum +1 증가
        seqNum++;
...
const input = '1 4 2 6 3 5'.split(' ').map((value) => parseInt(value, 10));

console.log(checkSeqNum(input) ? 'YES' : 'NO');
